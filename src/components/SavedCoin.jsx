  ...
  return (
    <div>
      {coins?.length === 0 ? (<p>You don't have any coins saved. Please save a coin to add it to watch list.
        <Link to=
    '/' className='text-accent'> Click here to save some coins.</Link>
        ...
        <tbody>
            {coins?.map((coin)=>{
                return(<tr key={coin.id} className='h-[60px] overflow-hidden'>
                    <td>{coin?.rank}</td>
                    ...
                    <td className='pl-8'><AiOutlineClose onClick={() => deleteCoin(coin.id)} className='cursor-pointer'/></td>
                    </tr>)
            })}
        </tbody>
      </table>) }
